...
exports.listBlogFromIdUser = async (req, res, next) => {
    if (!req.params.idUser) {
        return res.status(500).json({ success: false, message: 'Lỗi dữ liệu tải lên! ' });
    }
    let idUser = decodeFromAscii(req.params.idUser);
    try {
        let listBlogUser = await mdBlog.BlogModel.find({ idUser: idUser }).populate('idUser');
        if (listBlogUser) {
            return res.status(200).json({ success: true, data: listBlogUser, message: "" });
        }
        else {
