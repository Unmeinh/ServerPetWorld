...
exports.listAllBlog = async (req, res, next) => {
  ...
  let page =
    req.query?.page != undefined && Number(req.query?.page) == 0
      ? 1
  ...
  try {
    ...
    if (
      ...
      listAllBlog = await mdBlog.BlogModel.find()
        .populate([
          "idUser",
          {
            path: "idUser",
    ...
    if (listAllBlog) {
      ...
      return res.status(200).json({
        ...
        data: {
          ...
          canLoadMore:
            page != undefined && Number(page) < Number(totalPage)
              ? true
              : false,
        },
...
exports.listBlogFromIdUser = async (req, res, next) => {
  ...
  let limit = 2;
  let startIndex = (page - 1) * limit;
  let totalCount = await mdBlog.BlogModel.find({
    idUser: idUser,
  }).countDocuments();
  let totalPage = Math.ceil(totalCount / limit);
  ...
  try {
    ...
    if (
      ...
      listBlogUser = await mdBlog.BlogModel.find({ idUser: idUser })
        .sort({ createdAt: -1 })
        .populate("idUser");
    } else {
      if (isNaN(page)) {
...
exports.deleteBlog = async (req, res, next) => {
  ...
  if (req.method == "DELETE") {
    try {
      ...
      return res
        .status(203)
        .json({ success: true, data: {}, message: "Xóa blog thành công." });
    } catch (error) {
      return res
        .status(500)
        .json({ success: false, data: {}, message: "Lỗi: " + error.message });
...
exports.interactPost = async (req, res, next) => {
  if (req.method == "POST") {
    ...
    try {
      if (idBlog) {
        let objBlog = await mdBlog.BlogModel.findById({ _id: idBlog }).populate(
          [
            "idUser",
            {
              path: "idUser",
        ...
        return res
          ...
          .json({
            success: true,
            data: objBlog,
            message: "Đã tương tác với bài viết",
          });
      } else {
        return res
          .status(500)
          .json({ success: false, message: "Không đọc được dữ liệu tải lên!" });
      }
    } catch (error) {
      return res
        .status(500)
...
function getListWithFollow(blogs, loginId) {
  ...
  blogs.map((blog) => {
    ...
    let isFL = user.followers.find(
      (follower) => String(follower.idFollow) == String(loginId)
    );
    if (isFL) {
...
async function getBlogWithFollow(blog, loginId) {
  let blogWithFollow = {};
  let blogPopulate = await blog.populate([
    "idUser",
    {
      path: "idUser",
      populate: {
