...
exports.listCommentFromIdBlog = async (req, res, next) => {
  ...
  try {
    let listCommentByBlog = await mdComment.CommentModel.find({
      idBlog: idBlog,
    })
      .populate("idUser")
      .sort({ createdAt: -1 });
    if (listCommentByBlog) {
      return res
        ...
        .json({
          success: true,
          data: listCommentByBlog,
          message: "Lấy danh bình luận theo bài viết thành công",
        });
    } else {
      return res
        .status(200)
    ...
    try {
      if (idBlog && content) {
        ...
        if (blog) {
          ...
          if (String(req.user._id) != String(blog.idUser?._id)) {
    ...
    try {
      if (idComment) {
        ...
        if (comment) {
          ...
          return res
            .status(203)
            .json({ success: true, data: {}, message: "Đã xóa bình luận" });
        }
      } else {
        return res
          .status(500)
...
exports.addCommentInComment = async (req, res, next) => {
  let idUser = req.user._id;
  let idBlog = req.params.idBlog;
  let listCommentBlog = await mdComment.CommentModel.find({ idBlog: idBlog })
    .populate("idUser")
    .populate("idBlog");
  if (req.method == "POST") {
    try {
      ...
      await mdComment.CommentModel.findByIdAndUpdate(
        { _id: listCommentUser._id },
        listCommentUser
      );
      ...
      return res
        ...
        .json({
          success: true,
          data: listCommentUser,
          message: "Đã phản hồi bình luận",
        });
    } catch (error) {
      return res
        .status(500)
