...
exports.cartUser = async (req, res, next) => {
  ...
  try {
    ...
    if (req.method == "POST") {
      ...
      let result = listCartUser.carts.some(item => item.idProduct._id.toString() === idProduct.toString());
      if(result){
...
exports.checkProduct = async (req,res,next) => {
  ...
  try {
    let listCartUser = await mdCart.CartModel.findOne({ idUser: _id });
    let result = listCartUser.carts.some(item => item.idProduct.toString() === idProduct.toString());
    return res.status(200).json({
      success: true,
      data: result,
      message: "Sản phẩm đã có trong giỏ hàng",
  ...
  try {
    if(fullName && phoneNumber && location && req.method === 'POST'){
      ...
      if(user){
        ...
        res.status(201).json({success:true,data:user,message:'Thêm địa chỉ thành công'})
      }else{
        res.status(500).json({success:false,message:'Lỗi 500:Không tìm thấy user'})
      }
    }else{
  ...
  try {
    if(req.method === 'PUT'){
      ...
      user.locationDelivery.map((item, index) => {
        return item._id.toString() === idLocation ? item.isSelected = true : item.isSelected = false 
      })
      await mdUser.UserModel.findByIdAndUpdate({_id},user)
        res.status(201).json({success:true,data:user,message:'chỉnh sửa địa chỉ thành công'})
      }
    }else{
      res.status(500).json({success:false,message:'Lỗi 500'})
    }
...
exports.editLocation = async (req,res) => {
  const {_id} = req.user
  const {fullName,phoneNumber,location,idLocation} = JSON.parse(req.body.data)
  console.log(JSON.parse(req.body.data));
  try {
    if(fullName && phoneNumber && location && idLocation && req.method === 'PUT'){
      const user = await mdUser.UserModel.findById(_id)
