  ...
  async function sendFCMNotification(token, role, message) {
    const title = role === 'client' ? 'Thông báo từ Người mua' : 'Thông báo từ Người bán';
    const fcmData = {
      notification: {
        title: title,
        body: message
    ...
    const headers = {
      'Authorization': 'key=AAAA7kKbW5U:APA91bFwY7iSyzatKfVc0WBWh0gRYWu_uXLPfNo4FZHUCnmwCCirCGNchC7yFE2tTsRDoJFlNYBuD05WdANrDL38nX_pa-t8Xl2649Ak0g7mqAARFRszc4CDPkYrwADcJ4SJuryXwJDS',
      'Content-Type': 'application/json'
    };

    try {
      const response = await axios.post('https://fcm.googleapis.com/fcm/send', fcmData, { headers });
