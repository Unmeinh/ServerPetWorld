...
exports.listBillProduct = async (req, res, next) => {
  ...
  try {
    if (req.method === "GET") {
      let totalCount = await mdbillProduct.billProductModel.countDocuments(
        filterSearch
      );
      const totalPage = Math.ceil(totalCount / perPage);
      let skipCount = perPage * (currentPage - 1);
      let listBillProducts = await mdbillProduct.billProductModel
        .find({})
        .skip(skipCount)
        .limit(perPage)
        .exec();
...
exports.listShipper = async (req, res, next) => {
  ...
  try {
    if (req.method === "GET") {
      const billProduct = await mdbillProduct.billProductModel.findById(
        idBillProduct
      );
      ...
      let totalCount = await mdShipper.ShipperModel.countDocuments(
        filterSearch
      );
      const totalPage = Math.ceil(totalCount / perPage);
      let skipCount = perPage * (currentPage - 1);
...
exports.detailAdmin = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ message: "Shipper assigned to bill product" });
  } catch (err) {
    return res.status(500).json({ error: "Internal server error" });
  }
};
