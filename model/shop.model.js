...
ShopSchema.methods.generateAuthToken = async function (shop) {
    const token = jwt.sign({ _id: shop._id, userName: shop.userName }, string_word_secret);
    shop.token = token;
    return token;
}

ShopSchema.methods.encodeOwnerIdentity = async function (shop, identity) {
    const encode = jwt.sign({ _id: shop._id, ownerIdentity: identity }, string_word_secret);
